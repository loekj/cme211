...
class Students(object):
	...
	def dropMin(self)
		for sunet in self.student_dict.keys():
			...
			# drop idx num
			self.student_dict[sunet].pop(self.student_dict[sunet].index(min(score_list)))
	...
	def getProjectScore(self, weight):
		for sunet in self.student_dict.keys():
			self.student_dict[sunet][2] = weight*(self.student_dict[sunet][2]+self.student_dict[sunet][3])
			self.student_dict[sunet].pop(3)
	...
	def addTotal(self):
		...
		for sunet, grade_lst in self.student_dict.iteritems():
			self.student_dict[sunet].append(sum(grade_lst))
	...
	def addNormalized(self):
		...
		for sunet, scores in self.student_dict.iteritems():
			...
			if TA not in TA_scores:
				# -1 is total score minus minimum, quiz is idx 1
				TA_scores[TA] = [scores[-1] - scores[1]]
			else:
				TA_scores[TA].append(scores[-1] - scores[1])
		...
		for sunet, grade_lst in self.student_dict.iteritems():
			self.student_dict[sunet].append(self.student_dict[sunet][-1] + TA_adjust[self.map_TA[sunet]])
...
def main():
	...
	# Concat hw scores into one score times 0.5
	students.getHWscore(0.5)

	students.getQuizScore(0.25)

	students.getProjectScore(0.25)
